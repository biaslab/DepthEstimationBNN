...
export Unbounded, UnboundedSingle
...
struct UnboundedSingle{T1, T2, T3, P, TP}
    input_layer::T1
    hidden_layers::T2
    output_layers::T3
    prior::P
...
@functor UnboundedSingle (input_layer, hidden_layers, output_layers, posterior) # exclude prior from training


(m::Unbounded)(x_mean, x_var) = _applymodel(m, m.posterior, x_mean, x_var)
(m::UnboundedSingle)(x) = _applymodel(m, m.posterior, x)
...
@generated function _applymodel(model::UnboundedSingle, post::SafeTruncatedPoisson{MIN, MAX, T}, x) where {MIN, MAX, T}

    # create symbols
    state_symbols = vcat(:x, [gensym() for _ in 1:(MAX + 2)])
    output_symbols = [gensym() for _ in 1:(MAX - MIN + 1)]

    # create calls
    expand_call = :( $(state_symbols[2]) = model.input_layer($(state_symbols[1])) )
    state_calls = [:( $(state_symbols[i+2]) = model.hidden_layers[$i]($(state_symbols[i+1])) ) for i in 1:MAX+1]
